...
function adminVoteCountDisplay(p) {
  const raw = p.vote_count;
  const n = typeof raw === "string" ? Number(raw) : Number(raw);
  return Number.isFinite(n) ? Math.max(0, Math.floor(n)) : 0;
}

/** @param {Record<string, unknown>[]} photos */
async function enrichAdminPhotosVoteCounts(photos) {
  ...
  if (supabase) {
    try {
      ...
      if (!error && Array.isArray(data)) {
        for (const r of data) {
          const vcRaw = r.vote_count;
          const vc = typeof vcRaw === "string" ? Number(vcRaw) : Number(vcRaw) || 0;
          byId.set(String(r.photo_id), vc);
        }
      }
    } catch {
      byId = new Map();
  ...
  for (const p of photos) {
    const fromList = p.vote_count;
    const listNum = typeof fromList === "string" ? Number(fromList) : Number(fromList);
    const fromGal = byId.get(String(p.id));
    if (Number.isFinite(listNum)) {
      p.vote_count = Math.max(0, Math.floor(listNum));
    } else if (fromGal != null) {
      p.vote_count = fromGal;
    } else {
      p.vote_count = 0;
    }
  }
...
/** @param {Record<string, unknown>[]} photos */
function sortAdminPhotosList(photos, mode) {
  const arr = photos.slice();
  const idCmp = (a, b) => String(a.id).localeCompare(String(b.id));
  if (mode === "votes_desc") {
    arr.sort((a, b) => {
      const d = adminVoteCountDisplay(b) - adminVoteCountDisplay(a);
      return d !== 0 ? d : idCmp(a, b);
    });
  } else if (mode === "votes_asc") {
    arr.sort((a, b) => {
      const d = adminVoteCountDisplay(a) - adminVoteCountDisplay(b);
      return d !== 0 ? d : idCmp(a, b);
    });
  } else if (mode === "name_asc") {
    arr.sort((a, b) => {
      const na = String(a.full_name || "").toLocaleLowerCase();
      const nb = String(b.full_name || "").toLocaleLowerCase();
...
function getAdminSortMode() {
  const el = $("admin-sort");
  if (el instanceof HTMLSelectElement) return el.value || "server";
  return "server";
}

function renderAdminPhotosFromCache() {
  const box = $("admin-photos");
  if (!box) return;
  box.innerHTML = "";
  const sorted = sortAdminPhotosList(adminPhotosCache, getAdminSortMode());
...
function buildAdminEntry(p) {
  ...
  voteRow.className = "admin-entry__votes";
  const voteLbl = document.createElement("span");
  voteLbl.className = "admin-entry__votes-label";
  voteLbl.textContent = "Голосов";
  const voteNum = document.createElement("span");
  voteNum.className = "admin-entry__votes-num";
  voteNum.textContent = String(adminVoteCountDisplay(p));
  voteRow.appendChild(voteLbl);
  voteRow.appendChild(voteNum);
...
async function adminSaveOne(id, article, btn) {
  ...
  btn.disabled = true;
  try {
    const packed = packCaptionMeta(genderIn?.value, capIn?.value ?? "");
    await adminRpc("update_photo", {
      id,
      full_name,
      caption: packed,
    });
    const row = adminPhotosCache.find((x) => String(x.id) === String(id));
    if (row) {
...
async function adminDeleteOne(id, article) {
  ...
  try {
    ...
    adminPhotosCache = adminPhotosCache.filter((x) => String(x.id) !== String(id));
    article.remove();
...
async function adminLoad() {
  ...
  try {
    ...
    await enrichAdminPhotosVoteCounts(photos);
    adminPhotosCache = photos;
    renderAdminPhotosFromCache();
...
function wireAdmin() {
  ...
  $("admin-sort")?.addEventListener("change", () => renderAdminPhotosFromCache());
